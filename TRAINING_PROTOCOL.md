...
### 17.31 GTCRN 路线冻结与交付阶段（2026-07-24）

用户决定保持当前 GTCRN，不切换 DeepFilterNet。项目目标也随之从“继续提高低
SNR 抑制”切换为“保住当前听感并完成可部署验证”。当前冻结关系为：

```text
主观最佳候选：runs/classroom_v7_2_smoke/checkpoints/best.tar（epoch 1）
正式父模型：    runs/classroom_v7_1/checkpoints/best.tar（epoch 12）
选择理由：      v7.2 试听未发现人声损伤，优先级高于更强但会吞音的降噪
已知限制：      低 SNR machine 样本 04 仍有残噪和轻微时变调制
```

“主观最佳”不等于“已经完成部署验收”。v7.2 只在 400/80/80 smoke 上做了 repair，
不能为了获得“正式”名称而盲目扩大数据重训；扩大训练可能改变当前已经确认的人声
保护行为。冻结后不再进行以下工作：降低 SNR、提高抑制 loss、调整增益平滑、从头
训练、切换 DeepFilterNet。

下一步按以下顺序执行，且在第 1-3 步完成前不再训练：

1. **完整离线回归**：用 v7.2 epoch 1 重跑 v7.1 test 800 条、v4、v2、VoiceBank、
   中文 clean raw/norm 和 noise-only；与 v7.1 epoch 12 使用同一文件比较。
2. **扩大固定试听矩阵**：保留现有 13 条，再补充多个 HVAC-low、machine-low、
   far-low、murmur 和 identity 样本；任何新增吞音都否决 v7.2。
3. **冻结发布 checkpoint**：完整矩阵通过后复制并记录 SHA256、模型配置、Git
   commit 和测试摘要，不再用 `best.tar` 这种可被覆盖的路径作为发布标识。
4. **流式一致性验证**：当前训练与离线推理为 `center=true`，必须验证真实分块
   输入、帧缓存、边界连续性和离线/流式输出差异；只把 `center` 改成 false 不等于
   完成流式部署。
5. **Orange Pi CPU 基准**：先走 CPU/ONNX 或原生 PyTorch 对照，测实时因子、峰值
   内存和麦克风到扬声器总延迟；满足要求后再决定是否投入 RKNN/NPU 转换。

当前最紧迫的是第 1 步完整回归，而不是生成新训练集。若完整回归发现 v7.2 只在
13 条试听样本上看起来较好，则回退 v7.1 并接受其限制；若通过，才进入流式导出。

### 17.32 v7.2 完整离线回归（2026-07-23，17.31 第 1 步）

按 17.31 第 1 步，用 v7.2 epoch 1（`runs/classroom_v7_2_smoke/checkpoints/best.tar`）
在与 v7.1 epoch 12 完全相同的文件上重跑全部离线评估。命令脚本
`runs/v7_2_eval/run_full_regression.sh`，输出 `runs/v7_2_eval/full_regression/`，
对照方数据来自 `runs/v7_1_eval/`。

同文件对比（v7.1 -> v7.2）：

| 评估 | SI-SNR+ | PESQ+ | STOI+ | improved | noise-only att | clean SI-SNR | clean PESQ+ |
|---|---:|---:|---:|---:|---:|---:|---:|
| v7.1 test 800 | +2.99 -> +3.02 | +0.368 -> +0.365 | +0.0577 -> +0.0595 | 94.9% -> 95.6% | 24.34 -> 23.13 dB | 84.9 -> 93.9 dB | -0.063 -> -0.042 |
| v4 test 1000 | +0.252 -> +0.259 | +0.218 -> +0.190 | +0.0071 -> +0.0072 | 80.1% -> 81.6% | 16.15 -> 15.05 dB | 112.5 -> 111.9 dB | -0.0028 -> -0.0013 |
| v2 test 1000 | +0.487 -> +0.450 | +0.220 -> +0.185 | +0.0173 -> +0.0159 | 90.1% -> 91.3% | 16.68 -> 15.69 dB | 106.0 -> 105.6 dB | -0.0221 -> -0.0155 |
| VoiceBank 824 | +7.01 -> +6.79 | +0.538 -> +0.496 | +0.0043 -> +0.0058 | 99.2% -> 99.4% | — | — | — |
| zh clean raw 100 | — | — | — | — | — | 103.9 -> 112.3 dB | PESQ 4.641 -> 4.644 |
| zh clean norm 100 | — | — | — | — | — | 82.0 -> 90.3 dB | PESQ 4.570 -> 4.605 |

结论：

```text
1. v7.2 不是只在 13 条试听样本上好看：全部六个评估与 v7.1 持平或更好，
   目标域（v7.1 test）SI-SNR/STOI/improved fraction 全部略升。
2. clean 透明度全面明显改善：v7.1 test clean +9 dB，AISHELL raw +8.5 dB、
   norm +8.3 dB，clean PESQ 损伤在所有集合上约减半，与 smoke 结论一致。
3. 代价：noise-only 衰减少约 1-1.2 dB，PESQ 改善幅度略降（最大 -0.042，
   VoiceBank），v2 median SI-SNR+ 略降。均不构成否决。
4. 按 17.31 判定标准，v7.2 通过第 1 步完整回归，进入第 2 步扩大固定试听
   矩阵；v7.1 epoch 12 保留为回退版本。
```

### 17.33 波束成形前置假设与低 SNR 定位（2026-07-23）

用户提出并经讨论确认：部署链路中 GTCRN 的输入是波束成形输出，不是单麦原始
信号。32 通道天花阵列对扩散底噪的实际阵列增益预期 6-10 dB（理论上限约
15 dB），因此 GTCRN 实际看到的等效 SNR 大部分落在 v7.1 三档分布的
main/high 档（约 10-18 dB），1-4 dB extreme low 档在正常工作中很少出现。

据此明确低 SNR 训练数据的定位：

```text
低 SNR 档（尤其 1-4 dB）的意义是兜底不崩，不是追求降噪干净：
  - 波束指向教师时，学生区发言相对波束为远讲+离轴，等效 SNR 可掉到个位数
  - 定位跳变/跟踪滞后、回退固定宽波束或全向时，阵列增益瞬间消失
  - 扩散混响噪声场中波束增益打折
v7.2 当前取舍（低保人声、残噪可接受）与该定位一致，不再为加强低 SNR
抑制牺牲常见档位音质。
```

部署假设与联调验证项（并入 17.31 第 5 步前后的整机联调）：

```text
假设：GTCRN 输入为波束成形输出，等效 SNR 预期 >= 10 dB。
验证项 1：联调阶段实测波束后噪声底与等效 SNR 分布；若证实偏高，v7.2 只
          需输出混合比例/旁路微调，不重训。
验证项 2：用真实波束输出录音复测模型。已知失配：当前训练噪声为单麦形态
          平稳噪声，波束输出噪声带空间选择性、随波束跟踪呈非平稳；此项等
          波束成形链路稳定后用实采数据验证，现在不改训练数据。
```

### 17.34 扩大试听验收通过与发布冻结（2026-07-23，17.31 第 2-3 步）

扩大试听矩阵 31 组（原 13 + 新增 18：hvac_low/machine_low/far_low 各 4、
murmur 3、identity 3）位于 `runs/v7_2_eval/listening_expanded/`，逐文件指标
确认 v7.2 与 v7.1 在全部 31 文件上差异 <= 0.3 dB，无 v7.2 独有退化。

用户试听结论：低 SNR 持续噪声降噪不错、人声还原度高；其余情况降噪不干净或
有一定损伤，但 **v7.1 在同一文件上同样存在，且两模型听不出明显差别**。按
17.31 否决标准，无新增吞音，第 2 步通过。残留损伤判定为 GTCRN 路线共有
上限，记录在案，不在本轮解决。

第 3 步发布冻结完成：

```text
发布文件：release/gtcrn_classroom_v7_2_epoch1.tar
SHA256：  6f38816cf6d31a3578c699986d89b15101efc70ca1e4fdb6025a66dce24b472a
说明：    release/RELEASE_NOTES.md（配置、血缘、验收记录、已知限制、回退版本）
Git：     2f91172ad14ec6f7886871e34664832a54ed7e28
```

剩余步骤：第 4 步流式一致性验证、第 5 步香橙派 CPU 基准。两步通过后才算
完成部署验收。

### 17.35 流式一致性验证通过（2026-07-23，17.31 第 4 步）

新脚本 `gtcrn/verify_streaming_consistency.py`（不改动现有文件）对冻结 checkpoint
`release/gtcrn_classroom_v7_2_epoch1.tar` 完成离线 vs 流式对照。流式路径使用
`stream/modules/convert.py` 的缓存模块按 nfft=256 重组 StreamGTCRN（官方
gtcrn_stream.py 硬编码 nfft=512），前端为真实 5 ms 因果分块：每 80 样本取最近
160 样本加 sqrt-Hann 窗做 256 点 RFFT（不 center），模型内 conv/GRU 缓存逐帧
推进，IFFT + OLA（w^2 累积归一化）；启动补 80 零 pre-roll，结尾 flush 一个零
块，输出长度与输入完全一致，算法延迟 160 样本（10 ms）。

结果（`runs/v7_2_eval/streaming_check/`，含 REPORT.md、summary.json 和 4 对
可试听 wav）：

| 文件 | 稳态差异 SNR | 边界 max/mean vs 非边界 |
|---|---:|---|
| test_000054 machine_low | 30.67 dB | 0.051/0.0041 vs 0.086/0.0043 |
| test_000160 far_low | 33.49 dB | 0.046/0.0024 vs 0.071/0.0024 |
| test_000034 identity | 101.46 dB | 0.096/0.0072 vs 0.129/0.0074 |
| test_000156 hvac_main | 31.54 dB | 0.064/0.0084 vs 0.114/0.0080 |

结论：

```text
1. 一致性验收通过：稳态差异 SNR 全部 > 30 dB，无块边界爆音（边界一阶差分
   均不高于非边界位置），无 NaN/Inf，流式输出长度与输入零偏差。
2. 缓存实现被证明精确：流式模型逐帧跑离线帧与离线整段前向最大差约 1e-6
   （浮点误差量级），StreamConv/StreamTRA/DPGRNN 缓存与离线前向数学等价。
3. 残余波形差异全部来自因果前端（center=true 居中帧 vs 因果帧相差 48 样本
   循环移位的线性相位因子 + 边缘填充差异），属预期内，不影响部署。
```

第 4 步通过。剩余第 5 步：香橙派 CPU 基准（实时因子、峰值内存、麦克风到
扬声器总延迟），需在目标设备上执行。
